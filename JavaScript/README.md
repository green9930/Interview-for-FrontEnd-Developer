...
블록 레벨 스코프를 인정하는 let, const 키워드는 if문을 비롯한 모든 블록문에서 코드 블록이 실행되면 새로운 렉시컬 환경을 생성하여 기존 렉시컬 환경을 교체하고 if문이 종료되면 이전 렉시컬 환경으로 돌아간다.
...
클래스는 생성자 함수로 인스턴스 생성이 유일한 목적이므로 new 연산자와 함께 호출되어 인스턴스를 생성한다. (new 연산자 없이 클래스를 호출하면 타입 에러 발생)
